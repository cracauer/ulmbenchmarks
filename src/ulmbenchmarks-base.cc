...
public:
  ...
  double test_compression ()
  {
    // TODO:
    // Validate that a buffer filled with random values is not
    // compressible.
    return 1.0;
  }
} global_buf;
	     ...
	     , const char *funcname, long long size)
{
  ...
  if (size) {
    double ratio;
    ratio = global_buf.test_compression();
    if (ratio < 0.98)
      printf("compression ratio sucks: %.2f\n", ratio);
  }
#endif
  if (pf_msg_control == 1) {
...
// Taken from FreeBSD kernel.  Runs unconditionally even if hardware
// is not present.
static inline int rdrand(long *location)
{
  int count;
...
class randclass_rand {
public:
  typedef int argtype;
  static void doit(argtype *const where) {
    *where = rand();
  }
};

class randclass_rdrand {
public:
...
template <class R>
void randomize_buffer(gbuf &buffer, const size_t size)
{
  unsigned int i;
  typedef typename R::argtype T;
  T *loc;
...
class randclass_dev_urandom;
template <>
void randomize_buffer<randclass_dev_urandom>(gbuf &buffer, const size_t size)
{
  do_dev("/dev/urandom", buffer.buf, size, &cached_fd_urandom);
}
class randclass_dev_random;
template <>
void randomize_buffer<randclass_dev_random>(gbuf &buffer, const size_t size)
{
  do_dev("/dev/random", buffer.buf, size, &cached_fd_urandom);
}
class randclass_dev_zero;
template <>
void randomize_buffer<randclass_dev_zero>(gbuf &buffer, const size_t size)
{
  do_dev("/dev/zero", buffer.buf, size, &cached_fd_zero);
}


/*
Generate 16 bytes of random data and xor it across the buffer? (after it being "good" random to start with)
Non-initial generation should then be just the cost of getting it into, and out of, CPU cache. Once.
  Is it good enough to be seen as random by the compression? Well, if it compresses then no. Else probably yes.

  or block(N) ^= block(N-1) ^ this_randomness
    should be same cost, minus epsilon CPU cycles. Memory access is the cost.
*/


// taken from /dev/random on Linux
unsigned char high_quality_random_bytes[16] = {
  0x52, 0x8B, 0x5F, 0x92, 0x30, 0x0C, 0x7A, 0x28, 
  0xD6, 0x38, 0xE9, 0x0D, 0x2A, 0x59, 0x5A, 0x53
};

class randclass_walk16;
template <>
void randomize_buffer<randclass_walk16>(gbuf &buffer, const size_t size)
{
  const unsigned int blocksize = 16;
  if (size % blocksize != 0) {
    fprintf(stderr, "randclass_walk16 will only work with %d*n sized buffers\n", blocksize);
    return;
  }
  typedef unsigned char ptrtype;
  ptrtype *blockptr;
  blockptr = (ptrtype *)buffer.buf;

  if (size == 0)
    return;
  for (unsigned int i = 0; i < blocksize; i++) {
    blockptr[i] = high_quality_random_bytes[i];
  }

#if 0
  for (unsigned int block = 1; block < size / blocksize; block++) {
    int rnd = rand() % blocksize;
    for (unsigned int i = 0; i < blocksize; i++) {
      // Do not use ^=.  We have uninitialized data here and I don't want existing
      // randomness make this look better than it is
      blockptr[block * blocksize + i]
	= blockptr[(block -1) * blocksize + i]
	^ high_quality_random_bytes[rnd];
    }
  }
#endif

#if 1
  // walk bytes repeatedly
  for (unsigned int total = blocksize; total < size - blocksize; total++) {
    for (unsigned int i = 0; i < blocksize; i++) {
      blockptr[total] = blockptr[total - blocksize] ^ high_quality_random_bytes[i];
    }
  }
#endif
  ...
  for (unsigned int block = 1; block < size / blocksize; block++) {
    for (unsigned int i = 0; i < blocksize; i++) {
      blockptr[block * blocksize + i] = 
	blockptr[(block - 1) * blocksize + i] ^ high_quality_random_bytes[i];
      printf("meh %d %x %x %x\n", block
	     , blockptr[block * blocksize + i]
	     , blockptr[(block - 1) * blocksize + i]
	     , high_quality_random_bytes[i]
	     );
    }
  }
#endif
  ...
  for (unsigned i = 0 ; i < blocksize; i++)
    printf("%x/%x ", ((unsigned char*)buffer.buf)[i], high_quality_random_bytes[i]);
  printf("\n");
  for (unsigned i = blocksize ; i < blocksize * 2; i++)
    printf("%x/%x ", ((unsigned char*)buffer.buf)[i], high_quality_random_bytes[i % blocksize]);
  printf("\n");
  for (unsigned i = blocksize * 2; i < blocksize * 3; i++)
    printf("%x/%x ", ((unsigned char*)buffer.buf)[i], high_quality_random_bytes[i % blocksize]);
  printf("\n");
  for (unsigned i = blocksize * 3; i < blocksize * 4; i++)
    printf("%x/%x ", ((unsigned char*)buffer.buf)[i], high_quality_random_bytes[i % blocksize]);
  printf("\n");
  exit(0);
#endif
...
template <class T>
void test_rdbuf(const size_t size)
{
  global_buf.ensure_size(size);
  randomize_buffer<T>(global_buf, size);
}
...
void reestablish_mapping(int prot)
{
  ...
  if (touch_p) {
    // touch every 8th card, but only the 1st page for now (RAM savings)
...
{
  // write character to random location
...
int main(void)
{
  ...
  // FIXME
  // I left a note here:
  // "back to be changed pages with physical memory first"
  // WTH does that mean?
  ...
  if (getenv("ULM_MPROTECT_SIZE_IN_MB"))
    size_for_mprotect_tests = atoll(getenv("ULM_MPROTECT_SIZE_IN_MB"))
      * 1024ll * 1024ll;
  else {
    if (sizeof(long) == 4)
...
#if 1
  {
    int list[] = {4 * 1024, 2 * 1024 * 1024
		  // , 1024 * 1024 * 1024
		  , 0}; // zero terminates iteration
    for (int *it = list; *it; it++) {
      call_stuff_with_size(rdbuf<randclass_rand>, *it);
      if (getenv("DO_RDRAND"))
	call_stuff_with_size(rdbuf<randclass_rdrand>, *it);
      call_stuff_with_size(rdbuf<randclass_dev_urandom>, *it);
      if (getenv("DO_DEVRANDOM"))
	call_stuff_with_size(rdbuf<randclass_dev_random>, *it);
      call_stuff_with_size(rdbuf<randclass_dev_zero>, *it);
      call_stuff_with_size(rdbuf<randclass_walk16>, *it);
    }
  }
#endif
